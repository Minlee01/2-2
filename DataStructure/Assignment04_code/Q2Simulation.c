...
int max_serv_time = 3; // maximum service time for one customer
int clock;
...
void insert_customer(int arrival_time, char staff)
{
	element customer;

	customer.id = customers++;
	customer.arrival_time = arrival_time;
	customer.service_time = (int)(max_serv_time * random_num()) + 1;
	enqueue(&queue, customer);
	printf("Customer %d comes in table %c at %d minutes. Service time is %d minutes.\n", customer.id, staff, customer.arrival_time, customer.service_time);
}

// Retrieve the customer waiting in the queue and return the customer's service time.
int remove_customer(char staff)
{
	element customer;
	int service_time = 0;
	...
	printf("Customer %d starts service at table %c in %d minutes. Wait time was %d minutes.\n", customer.id, staff, clock, clock - customer.arrival_time);
	return service_time;
}
...
int main()
{
	...
	while (clock < duration) {
		...
		if (is_customer_arrived()) {
			insert_customer(clock, 'A');
		}

		// Check if the customer who is receiving the service is finished.
		if (A_service_time > 0) {	// the customer is receiving service
			A_service_time--;
			insert_customer(clock, 'B'); //new customer goes to B
		}

		// no customer is receiving service. 
		// So, take out a customer from the queue and start the service.
		else {
			A_service_time = remove_customer('A');
		}
		//if (is_customer_arrived('B'))

	// Check if the customer who is receiving the service is finished.
		if (B_service_time > 0)	// the customer is receiving service
			B_service_time--;
		else {
			B_service_time = remove_customer('B');
		}
