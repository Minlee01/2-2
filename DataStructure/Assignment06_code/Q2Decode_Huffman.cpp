...
void huffman_traversal(TreeNode* node, int* huff_arr, int count) {
    int index;
    if (node->l != NULL) {
        huff_arr[count] = 0;
        huffman_traversal(node->l, huff_arr, count + 1);
    }
    if (node->r != NULL) {
        huff_arr[count] = 1;
        huffman_traversal(node->r, huff_arr, count + 1);
    }
    if ((node->l == NULL) && (node->r == NULL)) {
        ...
        for (int j = 0; j < count; j++) {
            m_LUT[index][j] = huff_arr[j];
        }
        m_bit_size[index] = count;
    }
}
