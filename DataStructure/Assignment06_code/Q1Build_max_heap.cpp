...
void build_max_heap(HeapType* h)
{
	...
	for (int i = h->heap_size / 2; i > 0; i--) {
		int j = i;
		int k = i;
		while ((h->heap[i].key < h->heap[2 * i].key) || (h->heap[i].key < h->heap[(2 * i) + 1].key)) {
			if (h->heap[(2 * i) + 1].key > h->heap[2 * i].key) {
				temp.key = h->heap[i].key;
				h->heap[i].key = h->heap[(2 * i) + 1].key;
				h->heap[(2 * i) + 1].key = temp.key;
				//h->heap[i] = h->heap[(2 * j) + 1];
				//if (i <= h->heap_size / 2) {
				build_max_heap(h);
				//}
			}
...
int main()
{
	...
	for (int i = 0; i < input_size; i++)
		h1->heap[i + 1].key = rrandom(data_maxval);	// note) heap starts with 1.
	...
	printf("Runtime is %f", (double)(finish - start) / CLOCKS_PER_SEC);
}
